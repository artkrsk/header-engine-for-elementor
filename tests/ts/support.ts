...
/** Same capture-callback stub for MutationObserver. */
...
/**
 * A minimal container+bar pair matching the engine's expected markup, attached to the body.
 * happy-dom rects are all-zero and not scroll-aware, so both rects are stubbed: the bar to a real
 * height, the container to a scroll-tracking rect modeling a header whose document slot is 0 —
 * `measureNaturalTop` then resolves 0 at any scroll position, like a real top-of-page header.
 */
export const makeHeaderFixture = (barHeight = 80) => {
  ...
  container.getBoundingClientRect = () =>
    ({
      top: -window.scrollY,
      bottom: -window.scrollY + barHeight,
      height: barHeight
    }) as DOMRect
