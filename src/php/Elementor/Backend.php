...
class Backend {
	...
	public function add_customizer_settings( $wp_customize ): void {
		...
		$wp_customize->add_control(
			new \WP_Customize_Media_Control(
				...
				'arts_header_custom_logo_secondary',
				array(
					'label'         => esc_html__( 'Secondary Logo', 'header-for-elementor' ),
					'setting'       => 'arts_header_custom_logo_secondary',
					'priority'      => 8,
					'mime_type'     => 'image',
					'section'       => 'title_tagline',
					'button_labels' => array(
						'select'       => esc_html__( 'Select secondary logo', 'header-for-elementor' ),
						'change'       => esc_html__( 'Change secondary logo', 'header-for-elementor' ),
						'remove'       => esc_html__( 'Remove', 'header-for-elementor' ),
						'default'      => esc_html__( 'Default', 'header-for-elementor' ),
						'placeholder'  => esc_html__( 'No secondary logo selected', 'header-for-elementor' ),
						'frame_title'  => esc_html__( 'Select secondary logo', 'header-for-elementor' ),
						'frame_button' => esc_html__( 'Choose secondary logo', 'header-for-elementor' ),
					),
				)
			)
		);
	...
	public function update_theme_mods_on_site_settings_save( Document $document, $data ): void {
		...
		// Re-entrancy guard: the reverse-direction filters below fire on these
		// hooks; without it a theme-mod write could loop back through here.
