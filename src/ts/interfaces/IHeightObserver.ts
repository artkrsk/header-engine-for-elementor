...
export interface IHeightObserver {
  ...
  /**
   * Wired to the sticky engine's `onStickingChange`. Both edges suppress the live RO-driven
   * publishes for the bar's own transition and publish once, settled — per-frame root var writes
   * would style-recalc every consumer of the inherited height vars.
   */
