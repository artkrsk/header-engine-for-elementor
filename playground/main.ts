...
type Mode = 'flow' | 'overlay' | 'hero-bottom' | 'bottom'
type Reveal = 'off' | 'auto-hide'
...
/** Docking mode is a pure markup/CSS concern; options only carry behavior. Flow and overlay are
 * selected by the bar's modifier class, hero-bottom by the `_hero-bottom` wrapper class (it emits
 * no bar modifier and has its own rules in `_modes.scss`, so it does not reuse the overlay ones);
 * a `sticky.trigger` marker does the rest. */
function stickyFromState(): IStickyOptions | false {
  if (!state.stickyEnabled) {
    return false
  }
  const sticky: IStickyOptions = { reveal: state.reveal !== 'off' }
...
function mountFixture(headerContainer: HTMLElement): HTMLElement {
  ...
  if (state.mode === 'hero-bottom') {
    ...
    // flow / overlay: header is a body child before the content (natural position at the page top).
...
function rebuild(): void {
  ...
  // The reveal offset is a CSS var the engine resolves at measure time, not an option.
  container.style.setProperty('--arts-header-reveal-offset', `${state.revealOffset}px`)
...
function mountPanel(): void {
  ...
  const controls = [
    ...
    radioGroup(
      'mode (rebuild)',
      ['flow', 'overlay', 'hero-bottom', 'bottom'] as const,
      state.mode,
      setAndRebuild('mode')
    ),
    radioGroup(
      'reveal (rebuild)',
      ['off', 'auto-hide'] as const,
      state.reveal,
      setAndRebuild('reveal')
    ),
    slider('revealOffset', 0, 300, state.revealOffset, (v) => {
      state.revealOffset = v
      container?.style.setProperty('--arts-header-reveal-offset', `${v}px`)
      header?.refresh()
    }),
    radioGroup(
      'zone geometry',
